...
class AstronautsRestData: ObservableObject{
    @Published var astronauts:  [Astronaut] = []
    @Published var astronautFlights:  [Flight] = []
    static let shared = AstronautsRestData()
            ...
            if apiHttpsResponse.statusCode == 200{
                DispatchQueue.main.async {
                    do{
                        ...
                        self.astronauts = try decoder.decode(ApiResponse.self, from: validData).results
